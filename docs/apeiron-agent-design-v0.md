...
Apeiron 应该维护一份持续演化的项目记忆，并在正式编码前，用这份记忆准备聚焦的任务上下文。

第一版优先优化：

- 随时间理解一个仓库；
- 基于当前代码和项目记忆生成有用的任务上下文；
- 每次任务完成后自动更新项目记忆；
- 保持实现足够小，方便推理和迭代。
...
1. 构建一份紧凑的项目地图。
2. 将项目记忆存储在文件中。
3. 每个任务开始前，生成一份小型上下文包。
4. 只有需要时才读取真实源码文件。
5. 代码变化后，根据真实 diff 和测试结果更新记忆。
...
`.apeiron/ignore.md` 是 agent 的忽略规则配置。它会影响 warmup 和 work 的上下文选择，因此默认也应进入 git，让团队和仓库版本共享同一套 agent 忽略规则。
...
上下文包中的每个条目都应保留来源信息，方便用户和系统追踪内容从哪里来。
...
用户可以在 GUI 中临时排除某些 context item。被排除的条目不应进入接下来的模型上下文，但不会删除原始记忆或文件。
...
```text
1. 收集本回合被标记的 refresh targets。
2. 将 refresh targets 分为 modified / created / deleted / read-unread / read-documented。
3. 对 modified / created / deleted 文件调用 find_summary_for_file(path)。
4. 读取 summaryRef 指向的总结文档。
5. 根据 inventory.json 找到这些文件所属的模块和可能受影响的全局文档。
6. 找到可能受影响的 PROJECT / MODULES / CONVENTIONS / TESTING / MEMORY / ignore。
7. 读取相关源码、diff 和现有文档。
8. 检查本轮读取过的 unread 文件，执行 opportunistic warmup。
9. 逐个重申相关文档中的当前事实。
10. 写入 memory patch。
```
...
会决定 Apeiron 记忆模型、刷新语义、上下文选择和用户体验的部分，由 Apeiron 自己维护。
```
...
- context pack 生成和 context selection；
...
Apeiron 自己拥有 memory model、warmup/work/refresh flow、context pack 和用户命令。
```
